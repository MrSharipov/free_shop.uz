...
const getAuthorization = async (role, req, res, next) => {
	...
	try {
		...
		if (user.role !== role) {
			return res
				.status(403)
				.json({status: 403, message: 'You are not allowed to use this service'});
		}
		req.user = {
			_id: user._id,
			role: user.role,
		...
		return res.status(401).json({error: 'Authorization expired. Login again...'});
	}
};
...
const authentificate = async (req, res, next) => {
	...
	if (!user) {
		return res.status(404).json({error: 'User is not found: Invalid username'});
	}
	// check user's status
	if (user.blocked) {
		return res.status(403).json({error: 'This account has been blocked by admin'});
	}
	//check password
